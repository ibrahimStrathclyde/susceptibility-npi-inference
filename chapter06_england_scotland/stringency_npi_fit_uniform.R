...
# =============================================================================
# stringency_npi_fit_uniform.R
#
# Chapter 6: England and Scotland application
# Joint fit with a STRINGENCY-INDEX NPI, UNIFORM (flat) priors,
# heterogeneous susceptibility (Gamma, shared coefficient of variation nu).
#
# Sensitivity companion to stringency_npi_fit.R (hierarchical priors).
#
# Outputs:
#   outputs/stan_fits/fit_het_str_unif.rds
#  
#   outputs/figures/
#
...
prepare_joint_data <- function(filepath = "chapter06_england_scotland/data/GB_data.csv") {
  ...
  # ----- Analysis window: 31 Jan – 1 Jun 2020 -----
  ...
  # ----- Stringency indices for each country -----
  str_EN <- data$Stringency_EN[idx]
  str_SC <- data$Stringency_SC[idx]
  
  # In case of NAs, set to 0 (or you could do LOCF if preferred)
  str_EN[is.na(str_EN)] <- 0
  str_SC[is.na(str_SC)] <- 0
  
  # ----- Normalise and flip per country: u_t in [0,1] -----
  norm_flip <- function(x) {
    xmin <- min(x, na.rm = TRUE)
    xmax <- max(x, na.rm = TRUE)
    r    <- (x - xmin) / (xmax - xmin)   # in [0,1]
    u    <- 1 - r                        # flip
  ...
  # ----- Lockdown markers (for plotting only, still optional) -----
  ...
  list(
    england = list(
      ...
      t2 = t2_EN,
      population = 56e6,
      u = as.numeric(u_EN)   # <- flipped, normalised stringency
    ),
    scotland = list(
      ...
      t2 = t2_SC,
      population = 5.5e6,
      u = as.numeric(u_SC)   # <- flipped, normalised stringency
    )
  )
}
...
# ============================================================
#  Stan code 
#    c_t = cstar + (1 - cstar) * u_t, where u_t is flipped+normalised stringency
# ============================================================

stan_code_joint_str_uniform_het <- "
functions {
...
data {
  ...
  matrix<lower=0, upper=1>[C, T] u;  // flipped+normalised stringency in [0,1]
...
stan_data_hom <- within(as.list(stan_data_common), { CV_shared_fixed <- 0.0 })

# ============================================================
#  Compile + sample
# ============================================================
mod_het_str_unif <- stan_model("outputs/stan_fits/seir_nb_joint_stringency_uniform_HET_BASE.stan")



fit_het_str_unif <- sampling(
  object  = mod_het_str_unif,
...
rstan::check_hmc_diagnostics(fit_het_str_unif)

saveRDS(fit_het_str_unif, "outputs/stan_fits/fit_het_str_unif.rds")
...
# ============================================================
#  Choose fit for diagnostics output
# ============================================================
fit_use     <- fit_het_str_unif
model_label <- "Joint stringency:Uniform priors"
include_CV  <- TRUE
...
# ============================================================
# 9) Posterior summaries (country-level)
# ============================================================
tab_country <- tibble(
  Parameter = c("R0[1]","R0[2]","cstar[1]","cstar[2]","phi[1]","phi[2]","I0[1]","I0[2]",
                if (include_CV) "CV_shared" else NULL),
  Mean  = c(colMeans(post$R0), colMeans(post$cstar), colMeans(post$phi), colMeans(post$I0),
            if (include_CV) mean(post$CV_shared) else NULL),
  SD    = c(apply(post$R0,2,sd), apply(post$cstar,2,sd), apply(post$phi,2,sd), apply(post$I0,2,sd),
  ...
  Q2.5  = c(apply(post$R0,2,function(x) q(x)[1]),
            ...
            apply(post$I0,2,function(x) q(x)[1]),
  ...
  Q50   = c(apply(post$R0,2,function(x) q(x)[2]),
            ...
            apply(post$I0,2,function(x) q(x)[2]),
  ...
  Q97.5 = c(apply(post$R0,2,function(x) q(x)[3]),
            ...
            apply(post$I0,2,function(x) q(x)[3]),
            if (include_CV) q(post$CV_shared)[3] else NULL)
)


print(tab_country %>% mutate(across(where(is.numeric), ~round(. , 4))), n = Inf)
write.csv(tab_country, "posterior_summaries_joint_stringency_uniform.csv", row.names = FALSE)

# ============================================================
# 10) Fitted vs observed + NB predictive bands
# ============================================================
n_days <- length(dates)
...
print(p_fit)
ggsave("fitted_vs_observed_joint_stringency_uniform.png", p_fit, width = 10, height = 7, dpi = 300)
...
print(p_res_time)
ggsave("outputs/figures/residuals_vs_time_joint_stringency_uniform.png", p_res_time, width = 10, height = 6, dpi = 300)
ggsave("outputs/figures/residuals_vs_fitted_joint_stringency_uniform.png", p_res_fit, width = 10, height = 6, dpi = 300)
...
it <- dim(arr)[1]; ch <- dim(arr)[2]
par_names <- dimnames(arr)[[3]]

trace_df <- data.frame(
  Iteration = rep(1:it, times = ch*length(par_names)),
  Chain = factor(rep(rep(1:ch, each = it), times = length(par_names))),
...
print(p_trace)
ggsave("outputs/figures/trace_joint_stringency_uniform.png", p_trace, width = 12, height = 8, dpi = 300)
...
draw_list <- list(
  ...
  "I0[1]"    = as.numeric(post$I0[,1]),
  "I0[2]"    = as.numeric(post$I0[,2])
)
if (include_CV) draw_list[["CV_shared"]] <- as.numeric(post$CV_shared)
...
print(p_dens)
ggsave("outputs/figures/density_joint_stringency_uniform.png", p_dens, width = 12, height = 8, dpi = 300)
